...
export function Header({
  ...
  const logoContent = (
    // FIX: removed overflow-hidden so the dropdown is not clipped by this container
    <div className="relative min-w-0 flex-1">
      ...
      >
        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-2xl flex-shrink-0 flex items-center justify-center text-on-primary bg-primary shadow-[0_8px_18px_rgba(79,70,229,0.35)] dark:shadow-none transition-transform hover:scale-105">
        ...
        <div className="flex items-center gap-1 min-w-0 overflow-hidden">
          <h1 className="text-base sm:text-xl font-extrabold tracking-tight text-on-surface flex items-center gap-1 sm:gap-1.5 min-w-0">
            ...
            <span className="text-on-surface-variant font-medium text-sm sm:text-base truncate">
              {activeTab === "transcribe" ? "Transcribe" : "Translate"}
            </span>
          </h1>
          <ChevronDown className={cn(
            "w-3.5 h-3.5 sm:w-4 sm:h-4 text-on-surface-variant opacity-50 transition-all flex-shrink-0",
      ...
      {/* Dropdown Menu — rendered in a portal-like fixed position to escape any overflow:hidden ancestors */}
      <div className={cn(
        "absolute top-full left-0 mt-2 w-52 rounded-2xl border border-outline/20 bg-surface-container-high shadow-xl transition-all duration-200 z-50 overflow-hidden",
        showLogoMenu
        ...
        <button
          ...
          className={cn(
            "w-full flex items-center gap-3 px-4 py-3.5 text-sm font-bold transition-colors hover:bg-surface-container-highest",
        ...
        <div className="h-px bg-outline/10 mx-3" />
        <button
          onClick={() => {
  ...
  return (
    ...
    >
      <div className="max-w-4xl mx-auto px-3 sm:px-6 py-2">
        {/* FIX: removed overflow-hidden from this pill so the dropdown can escape it */}
